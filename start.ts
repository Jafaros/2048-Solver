...
const LogResults = (testName: string, results: Array<TestResult>) => {
	...
	console.log(
		`Test: ${testName}, Success Rate: ${((successCount / results.length) * 100).toFixed(2)}%, Average Score: ${averageScore.toFixed(2)}, Top Score: ${topScore}, Average Moves: ${averageMoves.toFixed(2)}`
	);
};
